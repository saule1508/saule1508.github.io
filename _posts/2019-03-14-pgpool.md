...
```
at the psql prompt, create a database (I use mydb), then quit
```
create database mydb encoding 'UTF8'  LC_COLLATE='en_US.UTF8';
\q
```
now reconnect to the newly create database and create two users (pierre_owner and pierre_user)
```
psql -U postgres mydb
create user pierre_owner nosuperuser nocreatedb login password 'pierre_owner';
create schema pierre_owner authorization pierre_owner;
create user pierre_user nosuperuser nocreatedb login password 'pierre_user';
grant usage on schema pierre_owner to pierre_user;
alter default privileges in schema pierre_owner grant select,insert,update,delete on tables to pierre_user;
alter role pierre_user set search_path to "$user",pierre_owner,public;
```
