...
* after install tfa will do an inventory, wait for it to be comple (see with tfactl status). In some case I had to restart tfa on one node in order to have the status ok on both nodes.
...
then do a run. Use screen or nohup (with -silentforce in this case) because it takes very long
...
the json is also in the directory upload.
